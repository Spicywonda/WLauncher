...
namespace WLauncher
{
    public class CLIHandler
    {
        ...
        {
            Console.WriteLine("Usage: WLauncher [command] [game name]");
            Console.WriteLine();
            ...
            Console.WriteLine();
            Console.WriteLine("  WLauncher --list");
            Console.WriteLine("  WLauncher --download Banjo64");
            Console.WriteLine("  WLauncher --run Banjo64");
            Console.WriteLine("  WLauncher -r \"Mario Kart 64\"");
            Console.WriteLine();
        }
                ...
                else if (executables.Count > 1)
                {
                    ...
                    try
                    {
                        ...
                        if (!string.IsNullOrEmpty(exePath))
                        {
                            ...
                            {
                                var possibleGuiExe = Path.Combine(exeDir, "WLauncher.exe");
        ...
        private async Task<int> UpdateLauncher()
        {
            try
            {
                ...
                string tempUpdateFolder = Path.Combine(Path.GetTempPath(), "WLauncher_temp_update");
        ...
        private static async Task CreateAndRunLauncherUpdaterScript(GitHubRelease release, string tempUpdateFolder, string tempDownloadPath, string currentAppDirectory)
        {
            ...
            string backupDir = Path.Combine(Path.GetTempPath(), "WLauncher_backup_" + DateTime.Now.ToString("yyyyMMdd_HHmmss"));
            string updateCheckFilePath = Path.Combine(currentAppDirectory, UpdateCheckFileName);

            if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
            {
                string updaterScriptPath = Path.Combine(Path.GetTempPath(), "WLauncher_Updater.cmd");
                string scriptContent = $@"@echo off
echo WLauncher Updater - Version {release.tag_name}
echo.
echo Waiting for WLauncher CLI to close...
set /A waitCount=0
:wait_loop
...
";
            ...
            {
                string updaterScriptPath = Path.Combine(Path.GetTempPath(), "WLauncher_Updater.sh");
...
if [ -f ""$appDir/WLauncher"" ]; then
    chmod +x ""$appDir/WLauncher""
    cd ""$appDir""
    nohup ""./WLauncher"" > /dev/null 2>&1 &
fi
