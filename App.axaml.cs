...
public class App : Application, INotifyPropertyChanged
{
    ...
    private bool ValidateUpdateFiles(string updateDirectory)
    {
        try
        {
            ...
            {
                mainExecutable = Path.Combine(updateDirectory, "WLauncher.exe");
            }
            else if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX))
            {
                ...
                {
                    mainExecutable = Path.Combine(updateDirectory, "WLauncher");
                }
            }
            else
            {
...
echo WLauncher Updater - Version {latestRelease.tag_name}
echo.

echo Waiting for WLauncher to close...
set /A waitCount=0
:wait_loop
