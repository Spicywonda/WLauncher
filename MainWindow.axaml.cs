...
namespace WLauncher
{
    public partial class MainWindow : Window, INotifyPropertyChanged
    {
                    ...
                    new Image
                    {
                        ...
                        Source = new Avalonia.Media.Imaging.Bitmap(
                            Avalonia.Platform.AssetLoader.Open(
                                new Uri("avares://WLauncher/Assets/CheckForUpdates.png"))),
        ...
        private void ValidateGameForm()
        {
            ...
            var repository = repoBox?.Text?.Trim() ?? string.Empty;
            var folderName = folderBox?.Text?.Trim() ?? string.Empty;
            ...
            else if (!repository.Contains("/") || repository.Split('/').Length != 2 || string.IsNullOrEmpty(repository.Split('/')[0]) || string.IsNullOrEmpty(repository.Split('/')[1]))
            {
                ValidationStatus = "Warning: Repository should be in the format 'username/repository'";
            }
        ...
        private async Task<string> FetchLatestCatalogTagAsync(string repo)
        {
            try
            {
                ...
                client.DefaultRequestHeaders.Add("User-Agent", "WLauncher");
...
private Border BuildCatalogCard(CatalogEntry entry, bool alreadyAdded)
        ...
        protected override void OnClosed(EventArgs e)
        {
            ...
            // Force process exit to ensure it closes completely
            Environment.Exit(0);
        }
                    ...
                    try
                    {
                        ...
                        var iconRect = new Avalonia.Controls.Shapes.Rectangle
                        {
                            ...
                            OpacityMask = new ImageBrush
                            {
                                Source = new Avalonia.Media.Imaging.Bitmap(
                                    Avalonia.Platform.AssetLoader.Open(
                                        new Uri($"avares://WLauncher/Assets/{iconPath}")))
                            }
                        };
        ...
        private async void CreateShortcut_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                WLauncher.Services.ShortcutHelper.CreateGameShortcut(
                    game,
                    launcherPath,
        ...
        private async void AddToSteam_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                string resultMessage = WLauncher.Services.ShortcutHelper.IsSteamRunning()
                    ? WLauncher.Services.ShortcutHelper.QueueGameAddToSteam(game, launcherPath)
                    : WLauncher.Services.ShortcutHelper.AddGameToSteam(
                        game,
                        launcherPath,
                        _gameManager.CacheFolder);
